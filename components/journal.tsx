  ...
  return (
    <section className={"flex flex-col gap-4"}>
      <div className="flex items-center gap-4">
        <time className="text-mono-11 shrink-0">
          {dayjs(props.date).format("dddd, MMMM Do")}
        </time>
        <div className="h-[1px] bg-mono-11 rounded-full w-full" />
      </div>
      {props.children}
    </section>
  );
...
function Entry(props: { time: string; children: React.ReactNode }) {
  return (
    ...
    >
      <time className="text-mono-11 font-semibold font-header text-sm mb-3">
        {props.time}
      </time>
      {props.children}
    </div>
