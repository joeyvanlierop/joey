  ...
  return (
    <figure className="group relative">
      <video
        ...
        className="cursor-pointer rounded-lg overflow-hidden transition-all bg-mono-5"
        style={{
          filter:
      ...
      <ControlPanel>
        ...
        <MuteControl
          muted={muted}
          onClick={() => emitControl(muted ? "unmute" : "mute")}
        />
        <LoopControl
          looping={looping}
          onClick={() => emitControl(looping ? "unloop" : "loop")}
        />
...
const PlayControl = ({ paused, onClick }) => (
...
const ProgressControl = ({ onClick, progress }) => (
  <div
...
const MuteControl = ({ muted, onClick }) => (
  <ControlButton onClick={onClick}>
    <svg
...
const LoopControl = ({ looping, onClick }) => (
  <ControlButton onClick={onClick}>
    <svg
      className="h-[14px] w-[14px] bg-white color-[hsla(0,0%,100%,.3)] transition duration-150"
      style={{
...
const MaximizeControl = ({ onClick }) => (
  <ControlButton onClick={onClick}>
    <svg
      className="h-[14px] w-[14px] bg-white"
      style={{ maskImage: MaximizeIcon, maskSize: "100% 100%" }}
    />
  </ControlButton>
);
